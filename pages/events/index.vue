            ...
            <Card style="overflow: hidden" class="bg-gray-800">
              <template #header>
                <div class="h-15rem overflow-hidden">
                  <img alt="user header" :src="event.eventBannerUrl" class="w-full" />
                </div>

              </template>
              <template #title>
                <div class="text-white">{{ event.eventName }}</div>
              </template>
              <template #subtitle>
                <div class="text-white">{{ getTime(event.eventStartDate) }}</div>
              </template>
              <template #content>
              ...
              <template #footer>
                <div class="flex gap-3 mt-1">
                  <GlobalButton v-if="event.eventAvailableTickets > 0" title="Buy Ticket" class="w-full"
                    @button-tapped="navigateTo('/events/' + event.eventId)" />
                  <GlobalButton v-else title="Sold Out" class="w-full soldOutButton" :disabled="true"
                    @button-tapped="navigateTo('/events/' + event.eventId)" />
                </div>
              </template>
            </Card>
